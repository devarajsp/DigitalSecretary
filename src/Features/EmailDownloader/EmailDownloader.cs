...
/// <summary>
/// Downloads a read-only COPY of every message (and its attachments) from all Yahoo IMAP
/// folders, mirroring the remote folder structure on disk. Messages are never deleted,
/// moved, or marked read on the server (folders are opened with EXAMINE / read-only access).
/// Each email is saved twice — as readable .txt and as raw .eml — plus all attachments.
/// </summary>
public sealed class EmailDownloader
{
    private const string Host = "imap.mail.yahoo.com";
        ...
        CancellationToken ct)
    {
        ...
        catch (AuthenticationException)
        {
            throw new InvalidOperationException(
                "Login failed. Yahoo requires an APP PASSWORD for IMAP access — your normal " +
                "account password will not work. Generate one at: Yahoo Account → Account " +
                "Security → \"Generate app password\", then paste it here.");
        }

        log.Report("Connected. Discovering folders…");
    ...
    private static string BuildLocalFolderPath(string root, IMailFolder folder)
    {
        ...
        foreach (var segment in folder.FullName.Split(delimiter, StringSplitOptions.RemoveEmptyEntries))
            path = Path.Combine(path, EmailFileNaming.Sanitize(segment));
        return path;
    }
    ...
    private static void SaveMessage(MimeMessage message, string dir, out int attachmentCount)
    {
        ...
        var attachments = AttachmentScanner.GetAttachmentEntities(message).ToList();
        attachmentCount = 0;

        var baseName = EmailFileNaming.BaseNameFor(message.Date, message.Subject);

        // Save the email in BOTH formats: a readable .txt and the full-fidelity raw .eml.
        var txtPath = EmailFileNaming.GetUniquePath(dir, baseName + ".txt");
        ...
        var emlPath = EmailFileNaming.GetUniquePath(dir, actualBase + ".eml");
        using (var emlStream = File.Create(emlPath))
            message.WriteTo(emlStream);

        foreach (var attachment in attachments)
        {
            var attachmentName = EmailFileNaming.Sanitize(AttachmentScanner.GetAttachmentName(attachment));
            var attachmentPath = EmailFileNaming.GetUniquePath(dir, $"{actualBase} - {attachmentName}");
    ...
    private static string BuildText(MimeMessage m, List<MimeEntity> attachments)
    {
        ...
        if (attachments.Count > 0)
            sb.AppendLine($"Attachments: {string.Join(", ", attachments.Select(AttachmentScanner.GetAttachmentName))}");
