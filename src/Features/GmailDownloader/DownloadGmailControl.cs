...
/// <summary>Downloads a local copy of all Gmail folders/labels, emails (.txt + .eml) and attachments.</summary>
public sealed class DownloadGmailControl : UserControl
{
    private readonly GmailSettingsStore _settingsStore;
    ...
    public DownloadGmailControl(IFeatureContext context)
    {
        _settingsStore = new GmailSettingsStore(context.DataDirectory);
        ...
        var defaultDir = string.IsNullOrWhiteSpace(saved.DownloadDir)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "DigitalSecretary_Gmail")
            : saved.DownloadDir;
        ...
        var note = new Label
        {
            ...
            Text = "Gmail requires an APP PASSWORD for IMAP — your normal password will not work.\n" +
                   ...
                   "This downloads a COPY only; your emails stay in Gmail, untouched."
        };
        ...
        form.Controls.Add(MakeLabel("Gmail address:"), 0, 0);
    ...
    private async void OnStart()
    {
        ...
        {
            MessageBox.Show("Please enter your Gmail address and app password.", "Missing details",
        ...
        _settingsStore.Save(new GmailSettings { Email = email, DownloadDir = dir });
        ...
        try
        {
            var downloader = new GmailDownloader();
